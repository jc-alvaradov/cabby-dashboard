...
class ResultsTable extends Component {
  ...
  render(){ 
    const headers = ["Rating", "From", "To", "Message", "Date", "Delete"].map((header) => <th>{header}</th>);
    const results = this.props.objects.map((res) => {
      ...
      items.push(<td>{moment(date).format('DD/MM/YYYY')}</td>);
      items.push(<td><DeleteBtn id={res.id} type="ratings" reload={this.props.reload}/></td>);
      return <tr>{items}</tr>
    });
