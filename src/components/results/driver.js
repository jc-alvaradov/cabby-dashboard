...
class ResultsTable extends Component {
  ...
  render() {
    ...
    const results = this.props.objects.map(res => {
      ...
      items.unshift(
        <td>
          <Image src={img} width="40" height="40" circle />
        </td>
      );
      items.push(
        <td>
          <LocationBtn lat={res.location.lat} lng={res.location.lng} />
        </td>
      );
      items.push(
        <td>
          <EditBtn data={res} type="drivers" reload={this.props.reload} />
        </td>
      );
      items.push(
        <td>
          <DeleteBtn id={res.id} type="drivers" reload={this.props.reload} />
        </td>
      );
      return (
        <tr>
